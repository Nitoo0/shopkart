        ...
        ConstraintLayout(constraintSet = constraints, modifier = Modifier.padding(innerPadding).fillMaxSize(), animateChanges = true) {
            ...
            Card(modifier = Modifier
                ...
                Column(verticalArrangement = Arrangement.Center, horizontalAlignment = Alignment.Start, modifier = Modifier
                    ...
                    RowComp(title = "Items Price:", price = "ksh${ DecimalFormat("#,##,###").format(totalAmount.value.toDouble()) }", space = 100.dp)
                    RowComp(title = "Delivery Fee:", price = "ksh${ DecimalFormat("#,##,###").format((100 * cartList.size).toDouble()) }", space = 90.dp)
                    RowComp(title = "GST:", price = "18%", space = 160.dp)

                    //(totalAmount.value + 100 * cartList.size) Adding 100rs for each item in the list
                    RowComp(title = "Total Price:", price = "ksh${ DecimalFormat("#,##,###").format((gstPrice).toDouble()) }", space = 110.dp)
                }
            }
    ...
    Surface(modifier = Modifier
        ...
        Column(modifier = Modifier.fillMaxSize(), verticalArrangement = Arrangement.SpaceEvenly, horizontalAlignment = Alignment.CenterHorizontally) {
            ...
            Row(modifier = Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.Center) {
                ...
                Text(text = "Note: ksh100 fee is applied for all the items in the cart", modifier = Modifier.padding(start = 5.dp) ,style = TextStyle(fontSize = 13.sp, fontWeight = FontWeight.Normal, fontFamily = roboto), color = Color.Black.copy(alpha = 0.5f))
            }
